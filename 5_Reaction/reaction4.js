...
function AnswerCheck(formp){
    ...
    for(n = 0; n < formp.answer.length; ++n){    // 問題（解答入力欄）繰り返す
        ...
        // 正誤の判定　文字変更ver
        if(formp.answer[n].value.replace(/\s+/g, '') == formp.Canswer[n].value.replace(/\s+/g, '') || formp.answer[n].value.replace(/\s+/g, '') == "inti=1;i<=19;i++" || formp.answer[n].value.replace(/\s+/g, '')=="i>=11"){  
...
function Answer(formp){
    ...
    for(n = 0; n < formp.answer.length; ++n){  
       ...
       formp.Lanswer[0].value = "int i = 1; i < 20; i++　or　int i =1; i <= 19; i++";
       formp.Lanswer[1].value = "i > 10　or　i >=11";
