...
function AnswerCheck(formp){
    ...
    for(n = 0; n < formp.answer.length; ++n){    // 問題（解答入力欄）繰り返す
        ...
        // 正誤の判定　文字変更ver
        if(formp.answer[n].value.replace(/\s+/g, '') == formp.Canswer[n].value.replace(/\s+/g, '') || formp.answer[n].value.replace(/\s+/g, '')  == "inti=0;i<=3;i++"){  
...
function Answer(formp){
    ...
    for(n = 0; n < formp.answer.length; ++n){  
       ...
       formp.Lanswer[1].value = "int i = 0; i < 4; i++　or　int i = 0; i <= 3; i++";
